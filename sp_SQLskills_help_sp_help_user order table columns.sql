USE [master]
GO
/****** Object:  StoredProcedure [sys].[sp_help]    Script Date: 8/21/2023 5:04:10 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create or ALTER procedure dbo.[sp_help_user]
...
begin
	...
	begin
		...
		select
			...
			'Storage_type'	= type_name(system_type_id),
			'Length'		= max_length,
			'Prec'			= Convert(int,TypeProperty(name, 'precision')),
			'Scale'			= Convert(int,TypeProperty(name, 'scale')),
			'Nullable'			= case when is_nullable=1 then @yes else @no end,
			'Default_name'	= isnull(object_name(default_object_id), @none),
			'Rule_name'		= isnull(object_name(rule_object_id), @none),
			'Collation'		= collation_name
	...
	begin
		...
		exec sp_SQLskills_helpindex @objname
		print ' '
...
EXEC sys.sp_MS_marksystemobject 'sp_help_user';
GO
