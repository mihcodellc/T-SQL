...
--use Extract Data-tier Application from Tasks' list then on right click dbS node then Deploy Data-Tier Application
--use Powershell as dump on postgres
-- BUT WON'T WORK IF OBJECTS ARE BROKEN
...
--last updated 9/29/2022 by Monktar Bello: added use of QUOTENAME
...
INNER JOIN (
	...
	INNER JOIN sys.columns ON (sys.columns.[object_id] = sys.objects.[object_id]) AND (sys.columns.column_id = sys.foreign_key_columns.parent_column_id)
	) ForeignKeys ON (ForeignKeys.referenced_object_id = sys.objects.[object_id]) AND (ForeignKeys.referenced_column_id = sys.columns.column_id)
