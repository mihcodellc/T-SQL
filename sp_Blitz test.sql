...
sp_blitz @CheckProcedureCache=1;
...
SELECT * into #MyTempTable FROM OPENROWSET ('SQLOLEDB','Server=(local);TRUSTED_CONNECTION=YES;mbello','set fmtonly off EXEC dbo.[sp_Blitz] @OutputXMLasNVARCHAR = 1')
SELECT * into #MyTempTable FROM OPENROWSET ('SQLOLEDB','Server=developer15\SQL2012;TRUSTED_CONNECTION=YES;Database=ThinkHealth','set fmtonly off EXEC iThinkHealth.dbo.[sp_Blitz] @OutputXMLasNVARCHAR = 1')
...
go
sp_BlitzIndex @Mode=4, @OutputServerName='DEVELOPER15\SQL2012', @OutputDatabaseName='iThinkHealth', @OutputSchemaName ='dbo', @OutputTableName='BelloBlitzIndex'
GO
