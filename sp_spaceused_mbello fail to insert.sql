...
IF OBJECTPROPERTY(OBJECT_ID('sp_spaceused_mbello'), 'IsProcedure') = 1
	DROP PROCEDURE sp_spaceused_mbello;
GO
...
create procedure dbo.[sp_spaceused_mbello] --- 2003/05/19 14:00
...
as
begin
--7/12/2024 by mbello  : added try catch when inserting @detail_tmp_table
...
DECLARE @detail_tmp_table table(
			...
			declare @Objectworking varchar(100)
			set @Objectworking = 'Object working:' + @objname
...
BEGIN
	...
	*/
	...
	BEGIN
	   ...
	   begin catch
			 declare @troubleObject varchar(128) = OBJECT_NAME(@id)
			 ...
			 set @errorline = ERROR_LINE()
			 set @errormessage = ERROR_MESSAGE()
			 set @errormessage = @errormessage + '- Object Trouble' + @troubleObject
			 raiserror(14827, 16, 3, @errornumber, @errorseverity, @errorstate, @errorline, @errormessage)
	...
	*/
	...
	BEGIN
		...
		SUM (CAST (SUBSTRING(data, 1, CHARINDEX(' ', data)) AS bigint))/1024 AS data_MB,
		SUM (CAST (SUBSTRING(index_size, 1, CHARINDEX(' ', index_size)) AS bigint))/1024 AS index_size_MB,
		SUM (CAST (SUBSTRING(unused, 1, CHARINDEX(' ', unused)) AS bigint))/1024 AS unused_MB--, GETDATE(), DB_NAME() 
	FROM @detail_tmp_table
END
END
...
EXEC sys.sp_MS_marksystemobject 'sp_spaceused_mbello';
GO
