...
--!!!!!!!!!! IMPORTANT
--  Exec RmsAdmin.dbo.p_RMS_DataFileClosedToMax @WorryThreshold = 70
...
--1  
 ...
 his.sql_severity, his.retries_attempted, job.job_id 
  ...
  order by StartTime desc
   ...
   select 'DBCC CHECKDB in OLA Table '
   SELECT 
      [DatabaseName]
      ,[SchemaName]     
      ,[ErrorNumber]
  ...
  order by [DatabaseName], StartTime desc -- barman@pgdb-n1 <barman@PGDB-N1.rms-asp.com>
...
SELECT ISNULL(d.[name], bs.[database_name]) AS [Database], d.recovery_model_desc AS [Recovery Model], 
	...
	'rms-svc@sql-backups - barman@PGDB-N1.rms-asp.com ' as isCopyOverCheckEmailreceived
...
----**sizes of backup and compression ratio
--SELECT top 200 backup_finish_date,
--backup_size/1024./1024./1024. as backup_size_GB, compressed_backup_size/1024./1024./1024. as compressed_backup_size_GB , backup_size/compressed_backup_size as compressionRatio,
--database_name , is_snapshot, is_copy_only, recovery_model, is_readonly, has_backup_checksums, is_damaged, (sum(compressed_backup_size) over (partition by datepart(wk,backup_finish_date)))/1024./1024./1024. week_size_GB
--FROM msdb.dbo.backupset
--where 
--database_name = 'MedRx'
--and 
--[type] = 'D' and is_snapshot = 0 and is_copy_only = 0 --to not include snapshot did by IT before its happened
...
-- select name,physical_name, size/128.0 size, max_size/128.0 max_size,  
--CAST(FILEPROPERTY(name, 'SpaceUsed') AS int)/128.0 as UsedSpace FROM sys.database_files order by name

-- DataFileClosedToMax or exec RmsAdmin.dbo.p_RMSFileSize
	declare @WorryThreshold_perc tinyint =95 
	declare @WorryThreshold_size_inMB int =  500000 --500GB
	...
	[Perc_InternalUsed] [numeric](17, 6) NULL,
    ...
    select DB_NAME() as dbname, df.physical_name, df.name as logic_name,Isnull(ds.name,''N/A'') as FileGroup, 
		df.size/128.0 as size_MB, 
		CAST(FILEPROPERTY(df.name, ''SpaceUsed'') AS int)/128.0 as InternalUsedSpace,
		((CAST(FILEPROPERTY(df.name, ''SpaceUsed'') AS int)/128.0)/(df.size/128.0))*100 as Perc_InternalUsed,
	...
	SELECT 'select * from @fillUp order by dbname, logic_name, CurrentSizeOnDik_MB asc'

	SELECT *
	FROM @fillUp
	--where physical_name like 'U%'
	ORDER BY dbname, logic_name, size_MB desc

	SELECT 'Data files filled up, it can be an issue soon. DBA needs to worry about !!! ATTENTION  !!!'

	select top 100 physical_name, logic_name, size_MB/1000 as size_MB,
	case when size_MB > @WorryThreshold_size_inMB then 1 else 0 end OverSize,
	[HowCloseToSizeonDisk_perc] as Perc_InternalUsed from @fillUp
	where dateinsert > convert(datetime2,replace(convert(CHAR(10), GETDATE(), 112),'-',''))
	and (cast([HowCloseToSizeonDisk_perc] as int) > @WorryThreshold_perc or cast(size_MB as int) > @WorryThreshold_size_inMB)
    ...
    'MedRx_Data_6',
    'LockBoxData_1',
	'FG_File_Archive_MasterPayerHistory_1',
	'Globalscape_DB'
	    )
	ORDER BY dbname, logic_name, [HowCloseToSizeonDisk_perc]	
...
----history space available NOT inlcude all Drive on Prod
--use  rmsAdmin
...
--where Collection_Time >= dateadd(yy,-1,getdate()) and drive = 'G:\'
...
EXEC xp_readerrorlog 0, 1, N'Logging SQL Server messages in file' -- show creation date = tempdb creation date = sql restart date
-- or :config manager > sql engine > startup parameters > existing paramters
...
select 'ASP-SQL, ASP-SQL-LOGSHIP, ASP-SQL002, MSQLDB-DEV, WS-netmon'
...
select '********************.\CheckBackUp_Space.ps1 for short term backup and on ASP-SQL Drive- DBSUPPORT-2487 Free Space for Full Backup up to assumption of x2 at least or 3times of previous size of previous backups, experience on SQL0003. also look at size ate when start backup the last time, here last week ********************'
